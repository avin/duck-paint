...
const DrawPage = () => {
  ...
  useEffect(() => {
    (async () => {
        ...
        const handlePointerMove = e => {
          if (modeRef.current === 'FREE') {
            ...
            if (pickInfo.hit) {
              ...
              brush.position = pickInfo.pickedPoint;
              ...
              brush.rotationQuaternion = quaternion;
              if (pickInfo.pickedMesh.rotationQuaternion) {
                brush.rotationQuaternion = pickInfo.pickedMesh.rotationQuaternion.multiply(
                  quaternion,
                );
              } else {
                brush.rotationQuaternion = quaternion;
      ...
      engine.runRenderLoop(() => {
        ...
        // // brushPlus.position = BABYLON.Vector3.Zero();
        // // if(brush.rotationQuaternion){
        // //   const quaternion = brush.rotationQuaternion.clone();
        // //   quaternion.x = 0;
        // //   quaternion.y = 0;
        // //   brushPlus.rotationQuaternion = quaternion;
        // // }
        //
        ...
        // const matrix = brushPlus.getWorldMatrix();
        // // y += 0.5;
        // const local_pos = new BABYLON.Vector3(0, 0, 0);
        // brushPlus.position = BABYLON.Vector3.TransformCoordinates(local_pos, matrix);
        ...
        let quaternion;
        if (brush.rotationQuaternion) {
          quaternion = brush.rotationQuaternion.clone();
        } else {
        ...
        const matrix = brush.getWorldMatrix();
        const localPos = new BABYLON.Vector3(0, 0.01, 0);
        brushPlus.position = BABYLON.Vector3.TransformCoordinates(localPos, matrix);

        // brushPlus.lookAt(scene.activeCamera.position);

        const q1 = brushPlus.rotationQuaternion.clone();

        brushPlus.lookAt(brush.position);

        const q2 = brushPlus.rotationQuaternion.clone();

        brushPlus.rotationQuaternion = BABYLON.Quaternion.Slerp(q1, q2, 0.5);
        // console.log(brushPlus.rotationQuaternion);

        rotCam.position = brushPlus.position;
        rotCam.rotationQuaternion = brushPlus.rotationQuaternion;

        const ray = rotCam.getForwardRay();
        ...
        if (pickInfo.hit) {
          ...
          brush.position = BABYLON.Vector3.Lerp(brush.position, pickInfo.pickedPoint, 0.1);
          ...
          BABYLON.Vector3.CrossToRef(axis2, axis1, axis3);
          const tmpVec = BABYLON.Vector3.RotationFromAxis(axis3.negate(), axis1, axis2);
          const quaternion = BABYLON.Quaternion.RotationYawPitchRoll(tmpVec.y, tmpVec.x, tmpVec.z);
          brush.rotationQuaternion = quaternion;


          // if (pickInfo.pickedMesh.rotationQuaternion) {
          //   brush.rotationQuaternion = pickInfo.pickedMesh.rotationQuaternion.multiply(quaternion);
          // } else {
          //   brush.rotationQuaternion = quaternion;
          // }
        }
        ...
        // const tar = {
        //   x: scene.activeCamera.getTarget().x * 1 - pos.x,
        //   y: scene.activeCamera.getTarget().y * 1 - pos.y + 0.01,
        //   z: scene.activeCamera.getTarget().z * 1 - pos.z,
        // };
        //
        // const ray = new BABYLON.Ray(pos, tar);
        // const pickInfo = scene.pickWithRay(ray, mesh => {
        //   return mesh === duck;
        // });
        //
  ...
  console.log(new BABYLON.Vector3(1, 1, 1).add(new BABYLON.Vector3(1, 1, 1)));
